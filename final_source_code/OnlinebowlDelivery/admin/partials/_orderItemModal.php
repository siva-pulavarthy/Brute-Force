                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($myrow = mysqli_fetch_assoc($myresult)){
                                        ...
                                        $itemsql = "SELECT * FROM `bowl` WHERE bowlId = $bowlId";
                                        $itemresult = mysqli_query($conn, $itemsql);
                                        $itemrow = mysqli_fetch_assoc($itemresult);
                                        $bowlName = $itemrow['bowlName'];
                                        $bowlPrice = $itemrow['bowlPrice'];
                                        $bowlDesc = $itemrow['bowlDesc'];
                                        $bowlCategorieId = $itemrow['bowlCategorieId'];

                                        echo '<tr>
                                                <th scope="row">
                                                    <div class="p-2">
                                                    <img src="/OnlinebowlDelivery/img/bowl-'.$bowlId. '.jpg" alt="" width="70" class="img-fluid rounded shadow-sm">
                                                    <div class="ml-3 d-inline-block align-middle">
                                                        <h5 class="mb-0"> '.$bowlName. '</h5><span class="text-muted font-weight-normal font-italic d-block">$. ' .$bowlPrice. '/-</span>
                                                    </div>
                                                    </div>
                                                </th>
